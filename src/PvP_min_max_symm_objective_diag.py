...
def prepare_matrix(strategy_center_flat, strategy_border_flat, M):
    ...
    for i in range(M):
        if i * 2 < M - 1:
            strategy_center[M - i - 1, i] = strategy_center_flat[-i-1]
        elif i * 2 > M:
            strategy_center[M - i - 1, i] = 1 - strategy_center_flat[-(M - i)]
        else:
...
cnt2 = cnt = 49 - the number of variables in the the second vector
cnt1 = cnt + N / 2 - 1 = 56 - the number of variables in the the first vector
...
# 12106879.998128386
# strategy_center_flat = np.ones(cnt1, dtype=dtype) * 0.1
# strategy_border_flat = np.ones(cnt2, dtype=dtype) * 1

# 32.151874764517075
#strategy_center_flat = np.linspace(0, 1, cnt1)
#strategy_border_flat = np.linspace(0, 1, cnt2)

# random
#strategy_center_flat = np.random.rand(cnt1)
#strategy_border_flat = np.random.rand(cnt2)
...
strategy_center_flat = np.array([0.29843788, 0])
strategy_border_flat = np.array([0.29843788])

# 4.5
#strategy_center_flat = np.ones(cnt1, dtype=dtype) * 0.5
#strategy_border_flat = np.ones(cnt2, dtype=dtype) * 0.5
################################################

################################################
# N = 6
#strategy_center_flat = np.array([1, 2, 3, 4, 5, 6])
#strategy_border_flat = np.array([1, 2, 3, 4])
...
################################################
# N = 8
#strategy_center_flat = np.arange(cnt1) + 1
#strategy_border_flat = np.arange(cnt2) + 1
