import numpy as np
from PvP_min_max_symm_objective_diag import objective_function, prepare_matrix
N = 6
M = N - 1
dtype = np.float64
cnt2 = (M ** 2 - 1) // 4 - M // 2
...
strategy_center_flat = np.ones(cnt2, dtype=dtype) * 0.5
strategy_border_flat = np.ones(cnt1, dtype=dtype) * 0.5
...
def upd_status(progress_info):
    ...
    x = progress_info['x']
    if not x is None:
        strategy_center, strategy_border = prepare_matrix(x[:cnt1], x[-cnt2:], M)
...
def min_max_fun(x, need_all=False):
    strategy_center = x[:cnt1]
    strategy_border = x[-cnt2:]
    #mx, res1 = max_objective(strategy_border, M)
    #mn, res2 = min_objective(strategy_center, M)

    mx, res1 = random_search(max_objective, M, num_iter=len(strategy_center) * 2, strategy_center=None, strategy_border=strategy_border)
    mn, res2 = random_search(min_objective, M, num_iter=len(strategy_border) * 2, strategy_center=strategy_center, strategy_border=None)
...
def random_search(opt_func, M, num_iter=100, strategy_center=None, strategy_border=None):
    ...
    for i in range(num_iter):
        if strategy_center is None:
            strategy_center_0 = np.random.uniform(low=bound[0], high=bound[1], size=cnt1)
        else:
            strategy_center_0 = strategy_center
        if strategy_border is None:
            strategy_border_0 = np.random.uniform(low=bound[0], high=bound[1], size=cnt2)
        else:
...
def random_search_all(num_iter):
    ...
    init_ans = None
    for i in range(num_iter):
        strategy_center_flat = np.random.uniform(low=bound_init[0], high=bound_init[1], size=cnt1)
        strategy_border_flat = np.random.uniform(low=bound_init[0], high=bound_init[1], size=cnt2)
...
if __name__ == '__main__':
    ...
    be = time.time()
    res, init_res = random_search_all_parallel(progress_info_value, num_iter=(cnt1 + cnt2) * 2)
    f, mn, mx = min_max_fun(res.x, need_all=True)
    strategy_center, strategy_border = prepare_matrix(res.x[:cnt1], res.x[-cnt2:], M)
    en = time.time()
    status_updater.cancel()

    t_run = en - be
    
    with open('res_diag.txt', 'a') as file:
        print('N:', N, 'M:', M, 'cnt1:', cnt1, 'cnt2:', cnt2, 'bound:', bound, \
            'f:', f, 'mn:', mn, 'mx:', -mx, \
